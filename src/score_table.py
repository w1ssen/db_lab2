...
class Score_Table(QWidget):
    ...
    def init_ui(self):
        info = selects.select_score_all(self.db, self.cursor)


        leninfo = 3
        self.table = QTableWidget(len(info), leninfo)
        self.table.setHorizontalHeaderLabels(['学号', '课程号', '成绩'])
        ...
        self.student_name_input.setPlaceholderText('课程号')
        self.student_age_input = QLineEdit()
        self.student_age_input.setPlaceholderText('成绩')

        self.add_student_button = QPushButton('添加成绩')
        ...
        self.inputLayout.addWidget(QLabel('成绩:'))
        ...
        self.student_name_input1.setPlaceholderText('课程号')
        self.del_student_button = QPushButton('删除成绩')
        ...
        self.inputLayout1.addWidget(QLabel('课程:'))
    ...
    def delete_student(self):
        ...
        # 将学生信息插入数据库
        try:
            scores.delete_score(self.db, self.cursor, student_id, student_name)
            self.load_data()
            widget = QWidget()
    ...
    def add_student(self):
        ...
        if not (student_id and student_name and student_age):
            ...
            # 将学生信息插入数据库
        try:
            scores.add_score(self.db, self.cursor, student_id, student_name, student_age)
            self.load_data()
            widget = QWidget()
