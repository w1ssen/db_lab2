...
class Prize_Table(QWidget):
    def __init__(self, db, cursor):
        super().__init__()
        self.db = db
        self.cursor = cursor
        self.photo = None
        self.setWindowTitle('奖项')
        self.resize(500, 500)
        self.init_ui()

    def init_ui(self):
        info = selects.select_prize_all(self.db, self.cursor)

        leninfo = 3
        self.table = QTableWidget(len(info), leninfo)
        self.table.setHorizontalHeaderLabels(['学号', '奖项','日期'])
        ...
        self.student_name_input1.setPlaceholderText('奖项')
        ...
        self.inputLayout1.addWidget(QLabel('奖项:'))
    ...
    def delete_student(self):
        ...
        # 将学生信息插入数据库
        try:
            prizes.delete_prizetime(self.db, self.cursor, student_id, student_name)
            self.load_data()
            widget = QWidget()
    ...
    def add_student(self):
        ...
        if not (student_id and student_name and student_age):
            ...
            # 将学生信息插入数据库
        try:
            prizes.add_prizetime(self.db, self.cursor, student_id, student_name, student_age)
            self.load_data()
            widget = QWidget()
